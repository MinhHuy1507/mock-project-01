...
def validate_unique(df, columns):
    ...
    df = df.withColumn(
        temp_col,
        F.when(duplicate_condition & ~null_condition, f"unique({','.join(columns)})"),
    )
    return df, temp_col
...
def validate_l0_to_l1(df, config):
    ...
    for validation in config["l0_to_l1"]["validation"]:
        ...
        if rule == "unique":
            df, temp_col = function(df, columns)
            error_exprs.append(F.col(temp_col))
            temp_unique_cols.append(temp_col)
        else:
