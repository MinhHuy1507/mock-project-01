...
from configs import customers_config, products_config, orders_config, province_config, global_config
...
def rcv_to_l0(table, date_path):
    ...
    path_rcv = global_config["path"]["rcv"] + config["prefix"] + date_path + f"/{table}.csv"
    path_l0 = global_config["path"]["l0"] + config["prefix"] + date_path + f"/{table}.csv"
...
def l0_to_l1(table, date_path):
    config = CONFIGS[table]
    path_l0 = global_config["path"]["l0"] + config["prefix"] + date_path + f"/{table}.csv"
    path_l1 = global_config["path"]["l1"] + config["prefix"] + date_path + f"/{table}.parquet"
    path_audit = global_config["path"]["error_records"] + config["prefix"] + date_path + f"/error_{table}.csv"
